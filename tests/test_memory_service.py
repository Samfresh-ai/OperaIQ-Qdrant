...
def test_canonical_incident_wins_over_learned_copy_on_repeated_alert() -> None:
    service = IncidentMemoryService(
        settings=Settings(qdrant_url=":memory:"),
        embedder=KeywordEmbedder(),
    )
    ...
    first = service.resolve_alert(DEFAULT_ALERT)
    second = service.resolve_alert(DEFAULT_ALERT.model_copy(update={"alertId": "repeat-alert"}))

    assert first.learnedIncident.incidentId.startswith("learned-")
    assert second.match.incidentId == "inc-redis-econnreset-2026-05-21"
