...
import { type AuditPhase, createCollection, getDocument, insertDocument, sendEvent, updateDocument, updateOperaIQIncident, writeAuditEntry } from "@operaiq/qdrant-brain";
import { generateIncidentConclusion } from "./gemini.js";
import { executeRemediation } from "./tools/execute-remediation.js";
import { operaiqGetRunbook } from "./tools/operaiq-get-runbook.js";
import { operaiqGetServiceDependencyGraph } from "./tools/operaiq-get-service-dependency-graph.js";
import { operaiqSearchSimilarIncidents } from "./tools/operaiq-search-similar-incidents.js";
import { operaiqWritePostmortem } from "./tools/operaiq-write-postmortem.js";
import { queryQdrantMemory, type ServiceSignal } from "./tools/query-qdrant-memory.js";
import { invocationFailed } from "./tools/common.js";

export type OperaIQEventSink = (event: AgentEvent) => Promise<void>;

export const runOperaIQAgentInputSchema = z.object({
  incidentId: z.string().regex(/^[a-f\d]{24}$/i),
  orgId: z.string().min(1),
  alert: normalizedAlertSchema
});

export interface RunOperaIQAgentResult {
  incidentId: string;
  toolsCalled: string[];
...
function verifyWaitMs(overrideMs?: number): number {
  ...
  const parsed = Number.parseInt(process.env.OPERAIQ_VERIFY_WAIT_MS ?? "15000", 10);
...
async function emit(sink: OperaIQEventSink | undefined, incidentId: string, stepType: AgentStepType, message: string, payload?: Record<string, unknown>): Promise<void> {
  if (!sink) return;
...
function qdrantQueryForAlert(alert: NormalizedAlert): string {
  ...
  if (alert.incidentType === "operaiq_test_payment_redis_spike" || symptomText.includes("econnreset")) {
...
function remediationTarget(action: string | null, defaultService: string | undefined, graph: Awaited<ReturnType<typeof operaiqGetServiceDependencyGraph>>): string {
...
function maybeForceCrash(phase: AuditPhase, override?: string): void {
  if (((override ?? process.env.OPERAIQ_FORCE_CRASH_PHASE ?? "")).toUpperCase() === phase) {
...
function graphServices(serviceName: string | undefined, graph: Awaited<ReturnType<typeof operaiqGetServiceDependencyGraph>>): string[] {
...
function serviceDependencies(service: string, graph: Awaited<ReturnType<typeof operaiqGetServiceDependencyGraph>>): string[] {
  if (!graph) return [];
...
function rootCauseFromSignals(signals: ServiceSignal[], graph: Awaited<ReturnType<typeof operaiqGetServiceDependencyGraph>>): string | null {
...
function forcedVerifyFailures(override?: number): number {
  ...
  const parsed = Number.parseInt(process.env.OPERAIQ_VERIFY_FAILS_BEFORE_PASS ?? "0", 10);
...
export async function runOperaIQAgent(input: unknown, sink?: OperaIQEventSink): Promise<RunOperaIQAgentResult> {
  assertProductionSafeRuntime("OperaIQ agent");
  const parsed = runOperaIQAgentInputSchema.parse(input);
  ...
  const timeline: Array<{ timestamp: string; event: string; actor: "operaiq" | "operaiq" | "human" }> = [];
  ...
  try {
    ...
    await auditedPhase(
      ...
      async () => {
        await updateOperaIQIncident(parsed.incidentId, parsed.orgId, {
          status: "in_progress",
    ...
    const similarIncidents = await auditedPhase(
      ...
      async () => {
        ...
        const result = await operaiqSearchSimilarIncidents(rememberInput);
        await emit(
          sink,
          parsed.incidentId,
          "REMEMBER",
    ...
    await updateOperaIQIncident(parsed.incidentId, parsed.orgId, { bestSimilarityScore });
    ...
    const graph = await auditedPhase(
      ...
      async () => {
        ...
        const result = await operaiqGetServiceDependencyGraph(graphInput);
        ...
        if (upgrade) {
          ...
          await updateOperaIQIncident(parsed.incidentId, parsed.orgId, { affectedServices: services });
        }
        await emit(
          sink,
          parsed.incidentId,
    ...
    const runbook = await auditedPhase(
      ...
      async () => {
        ...
        const result = await operaiqGetRunbook(runbookInput);
        await emit(
          sink,
          parsed.incidentId,
          "RETRIEVE",
    ...
    while (!resolved) {
      ...
      if (!result.success) {
        ...
        await updateOperaIQIncident(parsed.incidentId, parsed.orgId, { remediationAttempts, verifyResults });
      } else {
    ...
    const postmortem = await auditedPhase(
      ...
      async () => {
        ...
        const result = await operaiqWritePostmortem({
          incidentId: parsed.incidentId,
          orgId: parsed.orgId,
          timeline,
          rootCause,
    ...
    invocationFailed("run_operaiq_agent", error);
