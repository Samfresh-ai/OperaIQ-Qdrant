import { z } from "zod";
import { findSimilarIncidents } from "@operaiq/qdrant-brain";
import { qdrantMemoryQuery } from "@operaiq/qdrant-mcp";
...
export const operaiqSearchSimilarIncidentsInputSchema = z.object({
...
function tokens(values: string[]): string[] {
  const stopwords = new Set(["operaiq", "test-timing", "service", "app", "prod", "alert"]);
  return [
    ...new Set(
      values
...
export async function operaiqSearchSimilarIncidents(input: unknown): Promise<SimilarIncident[]> {
  const parsed = operaiqSearchSimilarIncidentsInputSchema.parse(input);
...
export const operaiqSearchSimilarIncidentsDefinition: AgentToolDefinition = {
