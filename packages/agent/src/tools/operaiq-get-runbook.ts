import { z } from "zod";
import { qdrantMemoryPut, qdrantMemoryQuery } from "@operaiq/qdrant-mcp";
...
export const operaiqGetRunbookInputSchema = z.object({
...
export async function operaiqGetRunbook(input: unknown): Promise<RunbookResult | null> {
  const parsed = operaiqGetRunbookInputSchema.parse(input);
...
export const operaiqGetRunbookDefinition: AgentToolDefinition = {
  name: "get_runbook",
